...
<TD bgColor=white vAlign=top width=745 align=center>
	...
	<TBODY>
	<TR>
	<TD class=location height=30 width="95%" align=right><A href="/main.jsp">HOME</A> &gt; 골프장부킹 &gt; <SPAN class=location_b>사전신청예약</SPAN></TD>
	</TR>
	<TR>
	...
	<TD vAlign=top>
		...
		<TD align=center>
			...
			%>
					<A class=area href="reserve.jsp?menu=<%=menuNum%>&region=<%=rList.get(i).getRegion_seq()%>&date=<%=strDate %>"><%=rList.get(i).getRegion_name()%></A> <%=i+1 < rList.size()? "&nbsp;I &nbsp;":"" %>
			<% 
					}
				} 
			%>
