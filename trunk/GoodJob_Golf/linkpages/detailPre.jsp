...
<%
	...
	if(curDate == null || curDate.length() != 8){
		out.print("<script>");
		out.print("location.href='reserve.jsp?menu=2&gr_cd="+nhNum+"&username="+URLEncoder.encode(nhName, "utf-8")+"';");
		out.print("</script>");
		return;
	}
...
function DisplayMenu(vIdx){
	...
	switch(vIdx){
		...
		case 4:
			...
			$("#ifrmMap").attr("src", "/linkpages/ifrm_map.jsp?x=<%=glDto.getPoint_x()%>&y=<%=glDto.getPoint_y()%>");
			$("#destap4").show(); 
			break;
	}
	subAutoResizeIFrame();
...
function nextResDate(){
	location.href = "/linkpages/detail.jsp?menu=2&gr_cd=<%=nhNum%>&username=<%=URLEncoder.encode(nhName, "utf-8")%>&golf=<%=golfSeq%>&date=<%=eDate%>&cdate=<%=curDate%>";
	return false;
}

function preResDate(){
	location.href = "/linkpages/detail.jsp?menu=2&gr_cd=<%=nhNum%>&username=<%=URLEncoder.encode(nhName, "utf-8")%>&golf=<%=golfSeq%>&date=<%=preDate%>&cdate=<%=curDate%>";
}
                                      ...
                                      <TD colspan="5"><TABLE border=0 cellSpacing=1 cellPadding=2 width="100%" bgColor=#d1d3d4 align=center>
                                          <TBODY>
                                            ...
                                            <TR>
                                              ...
                                              <TD style="PADDING-LEFT: 10px" bgColor=white colSpan=3><INPUT class=mem_input id="email1" name="email1" size=15 value="<%=eMail1 %>">
                                                @
                                            ...
                                            <TR>
                                              ...
                                              <TD style="PADDING-LEFT: 10px" bgColor=white colSpan=3><TEXTAREA class=box_02 cols=90 rows=4 id="requestContent" name="requestContent"></TEXTAREA></TD>
                                            </TR>
                                            <TR>
                                              <TD bgColor=#aed247 width=700 colSpan=4></TD>
                                            </TR>
