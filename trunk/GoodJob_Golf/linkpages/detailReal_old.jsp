...
if(curDate == null || curDate.length() != 8){
	out.print("<script>");
	out.print("location.href='reserve.jsp?menu=1&gr_cd="+nhNum+"&username="+URLEncoder.encode(nhName, "utf-8")+"';");
	out.print("</script>");
	return;
}
...
if(listGolf == null || listGolf.size() != 1){
	out.print("<script>");
	out.print("location.href='reserveReal.jsp?menu=1&gr_cd="+nhNum+"&username="+URLEncoder.encode(nhName, "utf-8")+"';");
	out.print("</script>");
	return;
}
...
function DisplayMenu(vIdx){
	...
	switch(vIdx){
		...
		case 4:
			...
			$("#ifrmMap").attr("src", "http://www.goodjobgolf.com/linkpages/ifrm_map.jsp?x=<%=glDto.getPoint_x()%>&y=<%=glDto.getPoint_y()%>");
			$("#destap4").show(); 
			break;
	}
	subAutoResizeIFrame();
...
function nextResDate(){
	location.href = "/linkpages/detail.jsp?menu=1&gr_cd=<%=nhNum%>&username=<%=URLEncoder.encode(nhName, "utf-8")%>&golf=<%=golfSeq%>&date=<%=eDate%>&cdate=<%=curDate%>";
	return false;
}

function preResDate(){
	location.href = "/linkpages/detail.jsp?menu=1&gr_cd=<%=nhNum%>&username=<%=URLEncoder.encode(nhName, "utf-8")%>&golf=<%=golfSeq%>&date=<%=preDate%>&cdate=<%=curDate%>";
}
	...
	<TD vAlign=top align=center>
		...
		<TD vAlign=top width=412>
			...
			<TD class=cc_name vAlign=top>
				...
				<TD>&nbsp;</TD></TR>
				<TR>
				<TD class=normal>가격정보<span class='red_n'>(자세한 할인 금액은 아래 요일/시간 별로 확인해 주시기 바랍니다.)</span></TD></TR>
				</TR>
				<TR>
				<TD>
					...
					<TD class=normal_b bgColor=#f1f1f1 align=center height=22>주중</TD>
					<TD class=normal_b bgColor=#f1f1f1 align=center>주말</TD>
					</TR>
					<TR>
					<TD class=normal_s bgColor=white width="150" align=center>그린피 할인금액</TD>
					<TD class=saturday bgColor=white align=center height=25><%=commify(sp) %></TD>
					...
					<TD class=normal_s bgColor=white align=center height=25><strike><%=commify(np) %></strike></TD>
					<TD bgColor=white align=center><strike><%=commify(nw) %></strike></TD>
					</TR>
					</TABLE>
				</TD>
				</TR>
...
%>
						...
						<TD bgColor=white align=center><span class=red_s><%=commify((listPr.get(i).getNH_price() + listPr.get(i).getReal_nh_price()) * 4) %></span>원</TD>
