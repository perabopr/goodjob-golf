...
<TD bgColor=white vAlign=top width=745 height="650" align=center>
	...
	<TD vAlign=top>
		...
		<TD align=center>
			...
			%>
				<a class=area href="reserve.jsp?menu=3&gr_cd=<%=nhNum%>&username=<%=URLEncoder.encode(nhName, "utf-8")%>">전체보기</a> &nbsp;I&nbsp;
			<%		
				}
			...
			%>
					<A class=area href="reserve.jsp?menu=3&gr_cd=<%=nhNum%>&username=<%=URLEncoder.encode(nhName, "utf-8")%>&region=<%=rList.get(i).getRegion_seq()%>"><%=rList.get(i).getRegion_name()%></A> <%=i+1 < rList.size()? "&nbsp;I &nbsp;":"" %>
			<% 
					}
				} 
			%>
...
%>
					...
					<TD vAlign=bottom rowSpan=2 width=100 align=center><A href="detail.jsp?menu=3&gr_cd=<%=nhNum%>&username=<%=URLEncoder.encode(nhName, "utf-8")%>&pkSeq=<%=pkList.get(i).getPackage_seq() %>"><IMG border=0 align=absMiddle src="../../images/common/btn_regist_booking.gif" width=89 height=34></A></TD></TR>
					<TR>
...
%>
		...
		<TR>
		<TD align=center>&nbsp;</TD></TR></TBODY> <TR><td height="20" align="center"><img src="img_package_text.gif" width="650" height="40" border="0"></td>
